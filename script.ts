...
interface RouteResult {
    busStopName: string;            // 乗車するバス停名 (正門, 創大門, 栄光門)
    busDepartureTime: number;       // バスが発車する時刻（0時からの通算秒）
    buildingLeaveDeadline: number;  // 建物を出発しなければいけない期限（秒）
    walkTimeMinutes: number;        // 補正済みの徒歩時間（分）
    afterBusWalkTimeMinutes: number;// バス下車後から駅までの徒歩（分）
    totalDurationMinutes: number;   // 総所要時間（分）
    busRouteName: string;           // バス系統名 (直通, ひ02 など)
    destinationArrivalTime: number; // 目的地に到着する時刻（秒）
}

/** 時刻表データの1行を表すタプル型（拡張版データ対応） */
type BusTimetableRow = [
    routeType: number,
    jrArrivalTime: number,
    keioArrivalTime: number | null,
    seimonDepartureTime: number | null,
    soudaimonDepartureTime: number | null,
    eikoumonDepartureTime: number | null,
    ...dayFlags: number[] // 日(インデックス6)〜土(12) の運行フラグ(0 or 1)
];

type DestinationType = 'JR' | 'Keio';

// 建物から各門への通常徒歩時間（分） [正門, 創大門, 栄光門]
const WALKING_DATA_MAP: Record<string, [number, number, number]> = {
    "中央教育棟": [11, 7, 9],
    "本部棟": [16, 11, 3],
    "教育学部棟": [6, 14, 11],
    "理工学部棟": [1, 16, 15],
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const settingMemory = document.getElementById('SettingMemory') as HTMLSelectElement;
    const tableSMemory = document.getElementById('TimeTable') as HTMLSelectElement;
    ...
    searchForm.addEventListener('submit', async (e: Event) => {
        ...
        const destination = (stationToggle.getAttribute('data-destination') as DestinationType);
        const speedFactor = parseFloat(speedSelect.value);

        const baseWalkTimes = WALKING_DATA_MAP[building];
        if (!baseWalkTimes) throw new Error(`undefined ${building}`);
    ...
    function restoreSavedState(): void {
        ...
        // ③ 行き先（駅名）の復元
        const savedDestination = localStorage.getItem('bus_nav_last_destination') as DestinationType | null;
    ...
    function renderAllRoutes(data: RouteResult[], from: string, to: string, isNextDay: boolean): void {
        if (!resultsArea || !recommendRouteCard || !otherRoutesList) return;
...
function calculateSearchTimeAndDay(period: string): { baseSeconds: number; dayIndex: number } {
...
async function searchRoutesWorkflow(
    period: string,
    baseWalkTimes: [number, number, number],
    destination: DestinationType,
    speedFactor: number,
    limit: number
): Promise<{ routes: RouteResult[]; isNextDay: boolean }> {
...
async function findAvailableBusRoutes(
    ...
    const matchedRoutes: RouteResult[] = [];
    ...
    for (const bus of filteredBuses) {
        ...
        if (destinationArrivalTime < searchSeconds) continue;

        let bestRouteForThisBus: RouteResult | null = null;
        ...
        if (!bestRouteForThisBus) continue;
        matchedRoutes.push(bestRouteForThisBus);
        if (matchedRoutes.length >= limit) break;
    }
...
async function loadTimetable(): Promise<BusTimetableRow[]> {
    ...
    const data = await response.json() as BusTimetableRow[];
...
/** 系統数値IDを表示用の文字列に変換する */
function getBusRouteName(routeType: number): string {
    switch (routeType) {
        case 0: return "直通";
        case 1: return "16号01";
        case 2: return "ひ02";
...
function toHourString(timeInSeconds: number): string {
